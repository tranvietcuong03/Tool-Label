...
def save_polygons(user_id, idx):
    ...
    # Lấy tên folder chứa file JSON
    folder_name = os.path.basename(json_save_dir)

    # Cập nhật hoặc thêm thông tin vào DataFrame
    if user_id in df['user_id'].values:
        # Nếu user_id đã tồn tại, cập nhật đường dẫn cho user_id đó
        df.loc[df['user_id'] == user_id, 'path_labelled_file'] = folder_name
    else:
        # Thêm dòng mới nếu user_id không tồn tại
        df = pd.concat([df, pd.DataFrame([{'user_id': user_id, 'path_labelled_file': folder_name}])], ignore_index=True)
